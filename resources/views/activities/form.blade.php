...
<div class="bg-white p-8 rounded shadow max-w-xl mx-auto">
    <h2 class="text-xl font-semibold mb-4">{{ isset($activity) ? __('Editar atividade') : __('Nova atividade') }}</h2>
    <form method="POST" action="{{ isset($activity) ? route('activities.update',$activity) : route('activities.store') }}">
        ...
        <div class="mb-4">
            <label class="block text-sm font-medium">{{ __('Turma') }}</label>
        ...
        <div class="mb-4">
            <label class="block text-sm font-medium">{{ __('Prazo') }}</label>
